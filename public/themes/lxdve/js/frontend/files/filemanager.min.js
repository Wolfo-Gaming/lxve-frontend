...
var ActionsClass = function() {
    function ActionsClass(element, menu) {
    ...
    _createClass(ActionsClass, [{
        ...
        key: 'sanitizedString',
        value: function sanitizedString(value) {
            return $('<div>').text(value).html()
        }
    }, {
        key: 'folder',
        value: function folder(path) {
            var inputValue = void 0;
            if (path) {
                inputValue = path
            } else {
                var nameBlock = $(this.element).find('td[data-identifier="name"]');
                var currentName = decodeURIComponent(nameBlock.data('name'));
                var currentPath = decodeURIComponent(nameBlock.data('path'));
        ...
        value: function move() {
            var nameBlock = $(this.element).find('td[data-identifier="name"]');
            var currentName = decodeURIComponent(nameBlock.attr('data-name'));
            var currentPath = decodeURIComponent(nameBlock.data('path'));
            swal({
                type: 'input',
                title: 'Move File',
        ...
        value: function rename() {
            var nameBlock = $(this.element).find('td[data-identifier="name"]');
            var currentLink = nameBlock.find('a');
            var currentName = decodeURIComponent(nameBlock.attr('data-name'));
            var attachEditor = '\n            <input class="form-control input-sm" type="text" value="' + currentName + '" />\n            <span class="input-loader"><i class="fa fa-refresh fa-spin fa-fw"></i></span>\n        ';
            nameBlock.html(attachEditor);
            var inputField = nameBlock.find('input');
            var inputLoader = nameBlock.find('.input-loader');
            inputField.focus();
            inputField.on('blur keydown', function(e) {
                if (e.type === 'keydown' && e.which === 27 || e.type === 'blur' || e.type === 'keydown' && e.which === 13 && currentName === inputField.val()) {
                ...
                $.ajax({
                    ...
                    if (!_.isEmpty(currentLink)) {
                        var newLink = currentLink.attr('href');
                        ...
                        nameBlock.html(currentLink.html(inputField.val()))
                    } else {
                    ...
                    nameBlock.addClass('has-error').delay(2000).queue(function() {
        ...
        value: function download() {
            var nameBlock = $(this.element).find('td[data-identifier="name"]');
            var fileName = decodeURIComponent(nameBlock.attr('data-name'));
            var filePath = decodeURIComponent(nameBlock.data('path'));
        ...
        value: function _delete() {
            var nameBlock = $(this.element).find('td[data-identifier="name"]');
            var delPath = decodeURIComponent(nameBlock.data('path'));
            var delName = decodeURIComponent(nameBlock.data('name'));
            swal({
                type: 'warning',
                title: '',
        ...
        value: function toggleMassActions() {
        ...
        value: function toggleHighlight(event) {
            ...
            var item = $(event.currentTarget).find('input');
            if ($(item).is(':checked')) {
        ...
        key: 'highlightAll',
        value: function highlightAll(event) {
            var parent = void 0;
            var item = $(event.currentTarget).find('input');
            if ($(item).is(':checked')) {
        ...
        value: function deleteSelected() {
            ...
            var parent = void 0;
            var nameBlock = void 0;
            var delLocation = void 0;
        ...
        value: function decompress() {
            var nameBlock = $(this.element).find('td[data-identifier="name"]');
            var compPath = decodeURIComponent(nameBlock.data('path'));
            var compName = decodeURIComponent(nameBlock.data('name'));
            swal({
        ...
        value: function compress() {
            ...
            $.ajax({
                ...
                Files.list(compPath, function(err) {
                    if (err) return;
                    var fileListing = $('#file_listing').find('[data-name="' + data.saved_as + '"]').parent();
                    fileListing.addClass('success pulsate').delay(3000).queue(function() {
                ...
                swal({
                    ...
                    title: 'Whoops!',
                    html: true,
                    text: _this.sanitizedString(error)
                })
            })
        }
    }]);
...
var ContextMenuClass = function() {
    function ContextMenuClass() {
        _classCallCheck(this, ContextMenuClass);
        this.activeLine = null
    }
    _createClass(ContextMenuClass, [{
        key: 'run',
        value: function run() {
            this.directoryClick();
            this.rightClick()
        ...
        key: 'makeMenu',
        value: function makeMenu(parent) {
            $(document).find('#fileOptionMenu').remove();
            if (!_.isNull(this.activeLine)) this.activeLine.removeClass('active');
            var newFilePath = $('#file_listing').data('current-dir');
            if (parent.data('type') === 'folder') {
                var nameBlock = parent.find('td[data-identifier="name"]');
                var currentName = decodeURIComponent(nameBlock.attr('data-name'));
                var currentPath = decodeURIComponent(nameBlock.data('path'));
                newFilePath = '' + currentPath + currentName
            }
            var buildMenu = '<ul id="fileOptionMenu" class="dropdown-menu" role="menu" style="display:none" >';
            if (Pterodactyl.permissions.moveFiles) {
                buildMenu += '<li data-action="rename"><a tabindex="-1" href="#"><i class="fa fa-fw fa-pencil-square-o"></i> Rename</a></li>                           <li data-action="move"><a tabindex="-1" href="#"><i class="fa fa-fw fa-arrow-right"></i> Move</a></li>'
            }
            ...
            if (Pterodactyl.permissions.createFiles) {
                buildMenu += '<li class="divider"></li>                           <li data-action="file"><a href="/server/' + Pterodactyl.server.uuidShort + '/files/add/?dir=' + $('<div>').text(newFilePath).html() + '" class="text-muted"><i class="fa fa-fw fa-plus"></i> New File</a></li>                           <li data-action="folder"><a tabindex="-1" href="#"><i class="fa fa-fw fa-folder"></i> New Folder</a></li>'
            }
        ...
        value: function rightClick() {
            var _this = this;
            $('[data-action="toggleMenu"]').on('mousedown', function(event) {
            ...
            $('#file_listing > tbody td').on('contextmenu', function(event) {
                _this.showMenu(event)
            })
        }
    }, {
        ...
        value: function showMenu(event) {
            ...
            var Actions = new ActionsClass(parent, menu);
            if (Pterodactyl.permissions.moveFiles) {
                $(menu).find('li[data-action="move"]').unbind().on('click', function(e) {
                    e.preventDefault();
                    Actions.move()
                });
                $(menu).find('li[data-action="rename"]').unbind().on('click', function(e) {
                    e.preventDefault();
                    Actions.rename()
                })
            }
            if (Pterodactyl.permissions.copyFiles) {
                $(menu).find('li[data-action="copy"]').unbind().on('click', function(e) {
                    e.preventDefault();
                    Actions.copy()
                })
            }
            if (Pterodactyl.permissions.compressFiles) {
                ...
                $(menu).find('li[data-action="compress"]').unbind().on('click', function(e) {
                    e.preventDefault();
                    Actions.compress()
                })
            }
            if (Pterodactyl.permissions.decompressFiles) {
                ...
                $(menu).find('li[data-action="decompress"]').unbind().on('click', function(e) {
                    e.preventDefault();
                    Actions.decompress()
                })
            }
            if (Pterodactyl.permissions.createFiles) {
                $(menu).find('li[data-action="folder"]').unbind().on('click', function(e) {
                    e.preventDefault();
                    Actions.folder()
                })
            }
            if (Pterodactyl.permissions.downloadFiles) {
                ...
                $(menu).find('li[data-action="download"]').unbind().on('click', function(e) {
                    e.preventDefault();
                    Actions.download()
                })
            }
            if (Pterodactyl.permissions.deleteFiles) {
                $(menu).find('li[data-action="delete"]').unbind().on('click', function(e) {
                    e.preventDefault();
                    Actions.delete()
                })
            }
            $(window).unbind().on('click', function(event) {
                if ($(event.target).is('.disable-menu-hide')) {
                    event.preventDefault();
                    return
                }
                $(menu).unbind().remove();
                if (!_.isNull(_this2.activeLine)) _this2.activeLine.removeClass('active')
            })
        }
    }, {
        key: 'directoryClick',
        value: function directoryClick() {
    ...
    _createClass(FileManager, [{
        ...
        value: function list(path, next) {
            ...
            $.ajax({
                ...
                _this.loader(false);
                $('#load_files').slideUp(10).html(data).slideDown(10, function() {
                    ContextMenu.run();
                    _this.reloadFilesButton();
                    _this.addFolderButton();
                    _this.selectItem();
                    _this.selectAll();
                    _this.selectiveDeletion();
                    _this.selectRow();
                ...
                if ((typeof Siofu === 'undefined' ? 'undefined' : _typeof(Siofu)) === 'object') {
        ...
        key: 'reloadFilesButton',
        value: function reloadFilesButton() {
            var _this2 = this;
            $('i[data-action="reload-files"]').unbind().on('click', function() {
        ...
        value: function selectItem() {
            $('[data-action="addSelection"]').on('click', function(event) {
                event.preventDefault()
            })
        }
    }, {
        ...
        value: function selectAll() {
            $('[data-action="selectAll"]').on('click', function(event) {
                event.preventDefault()
            })
        }
    }, {
        key: 'selectiveDeletion',
        value: function selectiveDeletion() {
            $('[data-action="selective-deletion"]').on('mousedown', function(event) {
                new ActionsClass().deleteSelected()
            })
        }
    }, {
        key: 'addFolderButton',
        value: function addFolderButton() {
            $('[data-action="add-folder"]').unbind().on('click', function() {
        ...
        value: function selectRow() {
            $('#file_listing tr').on('mousedown', function(event) {
                if (event.which === 1) {
